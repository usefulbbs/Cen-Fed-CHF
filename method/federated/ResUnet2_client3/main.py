...
from tensorflow.keras.layers import Conv1D, MaxPooling1D,  UpSampling1D, ZeroPadding1D,multiply
...
def create_keras_model():
    ...
    #input1 = Reshape(target_shape=(500, 1))(input)
    # print(input1.shape)
    #x = ZeroPadding1D(padding=6)(input)

    encoder1_1 = Conv_res(input, 24, 3, padding='same', activation='relu', name='encoder1_1')  # (input)
    ...
    encoder1_2 = concatenate([up1_2, encoder1_1], axis=2, name='merge1')
    ...
    encoder2_2 = concatenate([up2_2, encoder2_1], axis=2, name='merge2')
    ...
    encoder1_3 = concatenate([up1_3, encoder1_1, encoder1_2], name='merge1_3', axis=2)
    ...
    encoder3_2 = concatenate([up3_2, encoder3_1], name='merge3_2', axis=2)
    ...
    encoder2_3 = concatenate([up2_3, encoder2_1, encoder2_2], name='merge2_3', axis=2)
    ...
    encoder1_4 = concatenate([up1_4, encoder1_1, encoder1_2, encoder1_3], name='merge1_4', axis=2)
    ...
    output4 = Conv1D(1, 3, activation='sigmoid', padding='same', name='output_4', kernel_initializer='he_normal',
                     kernel_regularizer=l2(1e-4))(encoder1_5)
    ...
    #output44 = last_step(output4, name='output_44')
    output44 = last_step(encoder1_5, name='output_44')

    model = Model(inputs=input, outputs=output44)
...
def evaluation(state, test_data, test_label):
    ...
    print('test:')
    loss, accurcy = keras_model.evaluate(x=test_data, y=test_label, batch_size=4)
...
for i in [NUM_CLIENTS]:
    client_num = i
    path = r'/home/b220/PycharmProjects/zxh/compelete-experiment/divide dataset same as LiJia_client_num=3/dataset'
    print(path)
    for j in [9,10]:
        ...
        for r in range(num_rounds):
            ...
            print('*************************************************************************************************')
        name3 = 'roc_' + str(j) + 'fold_' + 'divide equally-client_num = 3'
        ...
        tff.learning.assign_weights_to_keras_model(save_model, state.model)
        save_model.save(save_model_name)


        name1 = 'train_' + str(j) + 'fold_'+'divide equally-client_num = 3'
        name2 = 'val_' + str(j) + 'fold_'+'divide equally-client_num = 3'
