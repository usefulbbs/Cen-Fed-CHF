...
for i in [3,7]:
    ...
    '''
    train_chf_label = np.ones(train_chf_data.shape[0])
    test_chf_label = np.ones(test_chf_data.shape[0])
    train_nsr_label = np.zeros(train_nsr_data.shape[0])
    val_chf_label = np.ones(val_chf_data.shape[0])
    val_nsr_label = np.zeros(val_nsr_data.shape[0])
    test_nsr_label = np.zeros(test_nsr_data.shape[0])
    train_data=np.concatenate([train_chf_data,train_nsr_data])
    train_label=np.concatenate([train_chf_label,train_nsr_label])
    test_data = np.concatenate([test_chf_data, test_nsr_data])
    test_label = np.concatenate([test_chf_label, test_nsr_label])
    train_data = np.concatenate([train_data, test_data])
    train_label = np.concatenate([train_label, test_label])
    val_data = np.concatenate([val_chf_data, val_nsr_data])
    val_label = np.concatenate([val_chf_label, val_nsr_label])
    np.random.seed(2)
    ...
    mc = ModelCheckpoint(filepath='cnn_ed_val2000' + str(i) + '.h5', monitor='val_acc', mode='max', verbose=2,save_best_only=True)
